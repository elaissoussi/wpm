        ...
        function ready() {
            ...
            if (result != null) {
                ...
                if (stderror != "") {
                    ...
                    // init the package by two file Package.json and README.md
                    // create README file with a txt content 
                    //var freadme = File(path + nameFile + PS +'README.md');
                    //freadme.create();

                    //var fstream = TextStream(freadme, "write");
                    //fstream.write(readme_txt);
                    //fstream.close();
                    // create a package.json in the folder, fill iit with description 
                    var pjson = File(path + nameFile + PS + 'package.json');
                    pjson.create();


                    var pstream = TextStream(pjson, "write");
                    pstream.write(description);
                    pstream.close();

                    // create a main.js in the folder, fill iit with description 
                    var pjson = File(path + nameFile + PS + 'main.js');
                    pjson.create();
        ...
        var folder = Folder(path);
            ...
            folder.forEachFile(function(file) {
                ...
                for (var f in files) {
                    ...
                    // execute git add for file
                    var command = 'git  ' + operation + files[f];
                    var stdin = ""; // must be an empty String or buffer;
                    var result = SystemWorker.exec(BIC + ' "' + command + '"', stdin, path);
   ...
   // invalid repository 
        ...
        function status(path, callback) {
            ...
            function parseStatus(gitstatus) {
                ...
                for (var i = 0; i < output.length; i++) {
                    ...
                    if (line.indexOf('modified') > -1 || line.indexOf('new file') > -1 || line.indexOf('deleted') > -1) {
                    ...
                    if (result != null) {
                        ...
                        if (stderror) {
                            ...
                            for (var i = parseme.length - 1; i >= 0; i--) {
                                val = parseme[i]
                                if (val.split('\t')[0]) list[val.split('\t')[0]] = val.split('\t')[1].split(' ')[0];
                            }
        ...
        function push(path, remote, branch, callback) {
            ...
            if (repository(path)) {
                ...
                var command = 'git push ' + ((remote) ? remote : '') + ' ' + ((branch) ? branch : '');
                var stdin = "";
                var result = SystemWorker.exec(BIC + ' "' + command + '"', stdin, path);
